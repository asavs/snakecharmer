//! Configuration for the Anti-Synapse daemon.
//!
//! Persisted as TOML at `%LOCALAPPDATA%\AntiSynapse\config.toml`. Defaults are
...
impl Config {
    ...
    /// Config directory: `%LOCALAPPDATA%\AntiSynapse` (falls back to CWD).
    pub fn dir() -> PathBuf {
        let base = std::env::var_os("LOCALAPPDATA")
            .map(PathBuf::from)
            .unwrap_or_else(|| PathBuf::from("."));
        base.join("AntiSynapse")
    }
