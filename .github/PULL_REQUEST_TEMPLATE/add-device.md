...
| `diagram` | `None`, or `Some(Diagram { width, height, shapes: &[...] })` | **Optional — `None` is fine and ships.** Without one the settings window uses labeled control rows and the device is fully functional. Drawing it is a separate skill and a separate PR, so a protocol port never waits on an illustration; when you do want to, the full workflow is [`docs/DRAWING-MICE-GUIDE.md`](../../docs/DRAWING-MICE-GUIDE.md). |
...
- [ ] *(Only if you drew a diagram)* Regenerated the SVG (`cargo test -p razer-proto -- --ignored regenerate_diagram_svgs`) and committed `docs/assets/<device>.svg`; the Button maps section embeds it and links the model's official schematic (no bundled PNGs, no trademarked logo). **Shipping `diagram: None` is a complete PR** — open a follow-up issue so someone can draw it later.
