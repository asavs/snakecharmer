# Installs the Psylli daemon autostart for the current user.
# Creates a hidden (windowless) launcher in the Startup folder that runs the
# release build of psylli.exe at login. Idempotent; no admin required.
#
# Reversible with uninstall-autostart.ps1.
$ErrorActionPreference = 'Stop'

$root = Split-Path $PSScriptRoot -Parent
$exe  = Join-Path $root 'target\release\psylli.exe'
